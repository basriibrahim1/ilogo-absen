...
const getAllReimbusement = () => {
    return pool.query(`
    SELECT reimbusement.id AS id, reimbusement.created_at AS reimbusement_created, reimbusement.nilai, reimbusement.catatan, reimbusement.kategori, users.name AS username, users.id AS user_id, reimbusement.tanggal, reimbusement.event, reimbusement.perusahaan FROM reimbusement 
    JOIN users ON reimbusement.user_id = users.id`)
}
const getReimbusementModels = (id) => {
    return pool.query(`
    SELECT * FROM reimbusement WHERE reimbusement.user_id = ${id}`)
}
